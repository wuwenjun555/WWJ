...
(function (win) {
  var
    ...
    has = function (a, o) { return isArr(a) && (a.includes(o)); },
    ...
    each = function (o, fn) {
      var i, l;
      if (isArr(o) || o instanceof A0) { for (i = 0, l = o.length; i < l; i++) { fn(i, o[i]); } return; } //if []
      for (i in o) { if (o.hasOwnProperty(i)) { fn(i, o[i]); } } //if {}
    },
  ...
  setORP(TQ, {
    clear: function () { tq = { stateFn: 0, sleep: 50, optSleep: 0, arrSleep: [], arrTask: [], curTask: 0, curRet: 0, isStop: 0, states: [], globaOpt: {}, overFn: 0 }; return TQ; },
    ...
    add: function (o) { $.isArr(o) ? (tq.arrTask = tq.arrTask.concat(o)) : (tq.arrTask.push(o)); return TQ; },
    start: function (fn) {
      (fn = function (to) {
        ...
        if (!to.hasShowState) {
          to.hasShowState = 1;
          if (tq.stateFn && !$.isNope(to.state)) { tq.stateFn(to.state, tq.curRet, tq.curTask.opt, tq.globaOpt); tq.arrTask.unshift(to); return setTimeout(fn, 20); }
        }
        if (!to.hasDone) {
          if (to.do) {
            try {
  ...
  setORP(WWJ, {
    ...
    has: has, each: each, setORP: setORP, A0: A0, ext: ext, TQ: TQ
  });
  ...
  WWJ({ WWJ: WWJ, is: 0, do: function (o) { return new A0(o); } });
})(window);
