...
(function (win) {
  ...
  $.setORP(DB, {
    hasDB: function (dbID) {
      return $.hasFile(dbBaseURL + dbID + '.json')
    },
    newDB: function (dbID) {
      var url = dbBaseURL + dbID + '.json';
      !$.hasFile(url) && ($.writeFile({ url: url, val: '{}' }));
    },
    getDB: function (dbID) {
      $.DB.newDB(dbID);
      return JSON.parse($.readFile({ url: dbBaseURL + dbID + '.json' }));
    },
    setDB: function (dbID, dbV) {
      $.writeFile({ url: dbBaseURL + dbID + '.json', val: JSON.stringify(dbV) });
    },
    bkDB: function (dbID) {
      if ($.DB.hasDB(dbID)) {
        $.writeFile({ url: dbBaseURL + 'zzzBK_' + dbID + '_' + $.getNow() + '.json', val: $.readFile({ url: dbBaseURL + dbID + '.json' }) });
      } else {
        $.DB.newDB(dbID);
      }
    },
    getDBIDs: function () {
      return $.getAllFileMainNames(dbBaseURL);
    },
    ...
    R: function (o) {
      var db = JSON.parse($.readFile({ url: dbBaseURL + o.dbID + '.json' })),
