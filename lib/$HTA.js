...
(function (win) {
  ...
  $.setORP($, {
    ...
    runCMD: function (cmd, isDebug) {
      // @demo 'cmd.exe /k node -v'
      var
        _isDebug = $.isVoid(isDebug) && $.debug,
        isKeepOrCloseWinWhenRunIsOver = _isDebug ? 'k' : 'c',
        isShowWin = _isDebug ? 1 : 0;
      $.getWS().run('cmd.exe /' + isKeepOrCloseWinWhenRunIsOver + ' ' + cmd, isShowWin);
    },
    openWeb: function (cmd, isDebug) { $.runCMD('start ' + cmd, $.isVoid(isDebug) ? 0 : isDebug); },
    ...
    // folder
    // TODO: use TQ rewrite
    eachFolder: function (url, fn, isDeep, isSubOnly) {
      var arr = [$.getFolder(url)], i;
      $.isVoid(isDeep) && (isDeep = 1);
      for (i = 0; i < arr.length; i++) {
        (isDeep || (!isDeep && i < 1)) && ($.eachFSO(arr[i].SubFolders, function (subFd) { arr.push(subFd); }));
        if (isSubOnly && i < 1) { continue; }
        fn(arr[i]);
      }
    },
    ...
    newFolder: function (url) {
      var a = [], u = url, r = 1;
      while (u.length > 0 && !$.hasFolder(u)) {
        a.push(u);
        u = u.substr(0, u.lastIndexOf('\\'));
      }
      while (a.length > 0) {
        r = r && $.getFSO().CreateFolder(a.pop());
      }
      return r;
    },
    renameFolder: function (url, nm) {
      if ($.hasFolder(url)) {
        var f = $.getFolder(url);
        f.Name = nm;
        return f;
      }
      return $.warn('The folder [' + url + '] is not exist! Skip rename folder');
    },
    bkFolder: function (url) {
      if ($.hasFolder(url)) {
        var f = $.getFolder(url);
        f.Name = f.Name + '_' + $.now();
        return f;
      }
      return $.warn('The folder [' + url + '] is not exist! Skip backup folder');
    },
